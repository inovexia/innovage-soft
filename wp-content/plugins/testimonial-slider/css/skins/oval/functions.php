...
function testimonial_post_processor_oval( $posts, $testimonial_slider_curr,$out_echo,$set,$data=array() ){
	...
	foreach($posts as $post) {
		...
		$html .= '<div class="testimonial_slideri" '.esc_attr($testimonial_slider_css['testimonial_slideri']).'>
			<!-- testimonial_slideri -->';
		...
		$testimonial_by_wrap='<div class="testimonial_by_wrap" '.esc_attr($testimonial_slider_css['testimonial_by_wrap']).'><span class="testimonial_avatar" '.$testimonial_slider_css['testimonial_avatar'].'><img src="'.esc_url($_testimonial_avatar).'" '.$testimonial_slider_css['testimonial_avatar_img'].' alt="' . esc_attr( strip_tags( $_testimonial_by ) ) . '"  /></span><div class="testimonial_by_text"><span class="testimonial_by" '.esc_attr($testimonial_slider_css['testimonial_by']).'>'.$_testimonial_by.'</span><span class="testimonial_site" '.esc_attr($testimonial_slider_css['testimonial_site_a']).'>'.$testimonial_company.'</span></div></div>';
...
function testimonial_slider_get_oval($slider_handle,$r_array,$testimonial_slider_curr,$set,$echo='1',$data=array()){
	...
	$slider_html.='<script>'.$fouc_dom.'</script>';
	$line_breaks = array("\r\n", "\n", "\r");
	$slider_html = str_replace($line_breaks, "", $slider_html);
	if($echo == '1')  {
		echo $slider_html; // Variable sanitised as constructed; cannot call wp_kses_post() here as it contains script.
	}
