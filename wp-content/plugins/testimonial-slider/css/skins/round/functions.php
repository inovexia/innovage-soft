...
function testimonial_post_processor_round( $posts, $testimonial_slider_curr,$out_echo,$set,$data=array() ){
	...
	foreach($posts as $post) {
		...
		if(!empty($content_limit)){ 
			$slider_excerpt = wp_kses_post(testimonial_slider_word_limiter( $slider_content, $limit = $content_limit));
		}
		...
		$slider_excerpt='<span '.esc_attr($testimonial_slider_css['testimonial_slider_span']).'> '.$slider_excerpt.'</span>';
		...
		if($testimonial_slider_curr['more'] and $testimonial_slider_curr['more']!='' and $permalink!=''){
			$read_more='<p class="more"><a href="'.esc_url($permalink).'" '.$testimonial_slider_css['testimonial_slider_p_more'].' '.$a_attr.'>'.$testimonial_slider_curr['more'].'</a></p>';
		}
		...
		if(isset($testimonial_slider_curr['show_star']) && $testimonial_slider_curr['show_star'] == 1 && $_testimonial_star != '' && $_testimonial_star > 0 ) {
			...
			for($i = 0; $i < $_testimonial_star; $i++ ) {
				$testimonial_quote.="<div class='dashicons dashicons-star-filled' ".esc_attr($testimonial_slider_css['dashicons-star-filled'])." ></div>";
			}
	...
	if($out_echo == '1') {
	   echo $html; // Variable sanitised as constructed; contains script so cannot use wp_kses_post() here.
	}
...
function testimonial_slider_get_round($slider_handle,$r_array,$testimonial_slider_curr,$set,$echo='1',$data=array()){
	...
	$testimonial_slider_css = testimonial_get_inline_css($set); 
	$slider_html='';
	$slider_handle = esc_attr($slider_handle);
	...
	if(!empty($sldr_title)) { 
	  $sldr_title = '<div class="sldr_title" '.esc_attr($testimonial_slider_css['sldr_title']).'>'. htmlspecialchars($sldr_title) .'</div>';
	}
	/*---- End Slider Title----*/
	// Scripts
	...
	$responsive_max_width=($testimonial_slider_curr['width']>0)?( $testimonial_slider_curr['width'].'px'  ) : ( '100%' );
		...
		$o_visible='{	min: 1,	max: '.((int)$testimonial_slider_curr['visible']) .'}';
		$o_responsive='responsive: true,';
		$o_width='width: '.((int)$testimonial_slider_curr['iwidth']).',';
	...
	if(!empty($testimonial_media_queries)){
			$script.='jQuery(function() {jQuery("head").append("<style type=\"text/css\">'. $testimonial_media_queries .'</style>");});';
	}
	$script.='jQuery(function() {
			jQuery("#'.$slider_handle.'").testiMonial({
				'.htmlspecialchars($o_responsive).'
				items: 	{
					'.$o_width.'
					visible     : '.esc_js($o_visible).'
				},
				'.$pagination.'
				auto: '.esc_js($autostep).','.$type.' '.$prevnext.'
				scroll: {
						items:'.htmlspecialchars($testimonial_slider_curr['scroll']).',
						fx: "'.htmlspecialchars($testimonial_slider_curr['transition']).'",
						easing: "'. htmlspecialchars($testimonial_slider_curr['easing']).'",
						duration: '.htmlspecialchars($testimonial_slider_curr['speed'] * 100) .',
						pauseOnHover: true
					}
			});
			jQuery("head").append("<style type=\"text/css\">#'.$slider_handle.'_nav a.selected{background-position:-'.htmlspecialchars($testimonial_slider_curr['navimg_w']).'px 0 !important;}.testimonial_slider__round .testimonial_outer_wrap:after {border-top-color: '.htmlspecialchars($testimonial_slideri_bg).'!important;}'.htmlspecialchars($border_top_color).' .testimonial_slider__round .testimonial_nav-fillup .inner_nav a { border: 2px solid '.htmlspecialchars($nav_color).' !important;}.testimonial_slider__round .testimonial_nav-fillup .inner_nav.selected a:after { background-color: '.htmlspecialchars($nav_color).' !important; }</style>");
	...
	<div id="'.$slider_handle.'_wrap" class="testimonial_slider testimonial_slider_set'. $set .' testimonial_slider__'.esc_attr($stylesheet).'" '.$testimonial_slider_css['testimonial_slider'].'>
		'.htmlspecialchars($sldr_title).'
		'.$nav_buttons_top.'
		'.$nav_top.'
	...
	if($echo == '1')  {
		echo $slider_html; // //phpcs:ignore WordPress.Security - String sanitised as construct, but potentially including generated script tag, so cannot use wp_kses
	}
