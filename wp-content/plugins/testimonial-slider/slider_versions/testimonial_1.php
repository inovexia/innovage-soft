...
function testimonial_global_posts_processor( $posts, $testimonial_slider_curr,$out_echo,$set,$data=array() ){
	...
	if(empty($skin) || preg_match('#[/\\\]#', $skin)) $skin='default';
	$set = esc_attr($set);
...
function testimonial_carousel_posts_on_slider($max_posts, $offset=0, $slider_id = '1',$out_echo = '1',$set='', $data=array() ) {
	...
	$posts = $wpdb->get_results($wpdb->prepare("SELECT * FROM 
		...
		ON a.post_id = b.ID 
		WHERE ( b.post_status = 'publish' AND b.post_type='testimonial' ) AND a.slider_id = %d 
		ORDER BY ".$orderby." LIMIT %d, %d", $slider_id, $offset, $max_posts), OBJECT);
...
global $testimonial_slider;
  ...
  // Settings page only
	if ( isset($_GET['page']) && ('testimonial-slider-admin' == $_GET['page'] or 'testimonial-slider-settings' == $_GET['page'] )  ) {
		...
		wp_localize_script('testimonial_slider_admin_js', 'testimonial_slider_admin', array('slider_update_review_me_nonce' => wp_create_nonce('slider_update_review_me_nonce')));
	...
	?>
        ...
        jQuery(function() {
				...
				foreach($sliders as $slider){ ?>
                    jQuery("#sslider_sortable_<?php echo esc_attr($slider['slider_id']);?>").sortable();
                    jQuery("#sslider_sortable_<?php echo esc_attr($slider['slider_id']);?>").disableSelection();
			    <?php } ?>
                });
        });
...
if($css and !empty($css)){?>
 <style type="text/css"><?php echo htmlspecialchars($css);?></style>
<?php }
}
