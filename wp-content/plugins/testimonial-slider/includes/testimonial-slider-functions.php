...
function testimonial_update_review_me() {
	 ...
	 if (empty($_POST['_slider_update_review_me_nonce']) || !wp_verify_nonce($_POST['_slider_update_review_me_nonce'], 'slider_update_review_me_nonce')) die('Security check.');
	 
	$testimonial_slider = get_option('testimonial_slider_options');
	if (!is_array($testimonial_slider)) $testimonial_slider = array();
	$reviewme = isset($_POST['reviewme']) ? stripslashes($_POST['reviewme']) : strtotime("now");
	if ($reviewme > 0) {
		$updated_reviewme = $testimonial_slider['reviewme']=strtotime("+1 week", $reviewme);
	} else {
		$updated_reviewme = $testimonial_slider['reviewme']=(int) $reviewme;	
	}
